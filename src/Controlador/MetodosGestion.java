...
public class MetodosGestion extends ManagerAbstract{
	...
	public ArrayList<Gestion> recogerGestion() throws SQLException {
		...
		while (resul.next()) {
			...
			gestion.setCodGestion(resul.getInt(1));
			gestion.setFecha(resul.getDate(2));
			gestion.setHora(resul.getTime(3));
			gestion.setCantidad(resul.getInt(4));
			
			String dniEmpleado = resul.getString(6);
			...
			String sql2 = "select distinct * from " + ManagerAbstract.TABLE_PRODUCTOS + " join " + ManagerAbstract.TABLE_GESTION + " on productos.CodProducto = gestion.CodProducto where gestion.CodProducto = '" + resul.getInt(codProducto) + "'";
		...
		for (Producto producto : listaproducto) {
			if(producto.getCodProducto() == codProducto) {
				resul.executeUpdate("Insert into `" + ManagerAbstract.TABLE_GESTION+ "`"
						+ "(`CodGestion`, `Fecha`, `Hora`, `Cantidad`, `CodProducto`, `DNI`) "
						+ "VALUES( '" + codGestion + "' , '"+ fecha + "' , '" + hora + "' , '" + cantidad + "' , '" 
						+ codProducto + "' , '" + dni + "');");
			}
		}
	}
	
	public void eliminarGestion(ArrayList<Gestion> listaGestion, int  codGestion) throws SQLException {

		Connection conexion;
		conexion = DriverManager.getConnection(DBUtils.URL, DBUtils.USER, DBUtils.PASS);
		Statement resul = conexion.createStatement();

		ArrayList<Gestion> listaGestionEliminar = listaGestion;

		for (Gestion gestion : listaGestionEliminar) {
			if (gestion.getCodGestion() == codGestion) {
				resul.executeUpdate(
						"DELETE FROM `" + ManagerAbstract.TABLE_GESTION + "`" + "WHERE gestion.CodGestion = '" + codGestion + "';");
			}
		}
	}
